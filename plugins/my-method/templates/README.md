# Instantiating a my-method template

The single contract for turning any template in this folder into a real project document. Every skill in this pack that writes a document follows this file — it exists once so the rules cannot drift between skills.
...
4. **Delete any repeatable stanza you have no content for.** Several templates carry a heading-plus-content pair meant to be copied per item or dropped entirely. A leftover `{{decision}}` in a finished document is worse than a missing section — the next session reads it as content.
