<!-- guidance: written fresh for each phase by start-session, and kept honest during the
...
<!-- guidance: observable statements, each confirmable by using the thing. If you cannot tell
     ...
     phase drifting, and it is what start-session reads back at the top of the next session. -->
...
<!-- guidance: the honest state, updated as work happens and trued up by wrap-session.
...
<!-- When this phase finishes, start-session copies this whole file to the top of
