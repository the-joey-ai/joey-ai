...
- **Real expertise beats general knowledge.** A skill synthesised from actual artefacts — the owner's corrections, past outputs, the project's own documents — outperforms one synthesised from generic best-practice content. The value lives in what's specific to the owner.
...
- **Add what the agent lacks; omit what it knows.** The agent knows what a JD is, how email works, what a QBR is for. Write the parts it would get wrong: the owner's standard, their structure, their naming, their historical mistakes. The per-line test: *would the agent get this wrong without this instruction?* No → cut.
...
3. **Acknowledge evolution.** Name the owner of the standard the skill encodes and frame the standard as current, not permanent. This gives the skill a basis for flagging its own staleness and a place to send feedback: the project's `ENHANCEMENTS.md`, or wherever the owner keeps what they want changed. A skill written as a final answer goes stale the day the standard moves, and nobody notices until the outputs are quietly wrong.
