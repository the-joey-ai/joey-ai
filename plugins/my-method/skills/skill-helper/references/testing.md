...
Design **two or three on-target prompts** plus **one off-target near-miss**, and get the user's sign-off before running — they know their colleagues' phrasing better than you do.

On-target prompts must be realistic, not ideal. Real users write casually, with context and clutter: file names, half-remembered details, typos, backstory. "Write a JD" tests nothing; "hey I need a job posting for a senior PM on the registry team, sarah has an old one from 2024 somewhere but the level is different now" tests the skill. Vary the phrasings — one formal, one casual, one that needs the skill without naming it.

The off-target prompt is the important one most builders skip. Make it a genuine near-miss — shares keywords or territory with the skill but should be handled *without* it ("draft an offer letter" against a JD skill). An obviously irrelevant prompt tests nothing. A skill that hijacks adjacent requests fails at scale even when every happy-path run is perfect.
...
- **Without subagents:** run each prompt yourself, following the skill's instructions as written — not as intended. Tell the user plainly this is weaker evidence (author and tester share a context).

**Triggering and off-target — only provable installed:** the real test is packaging the `.skill`, having the user install it, and running the on-target and off-target prompts in fresh chats. Until that's happened, never report a trigger or off-target result as a pass — say plainly that it's untested. If the user wants an early signal, one weak proxy exists: give a fresh subagent only the frontmatter description plus the test prompt and ask whether it would load that skill for that task. Label it a proxy in anything you report; it approximates the routing decision, it isn't one.
...
2. **Is it genuinely good?** Would the user hand this output to the colleague it's for, unedited? If the answer is "it's fine, I'd tweak it" — that's a fail at scale. Mediocre output multiplied across an org sets the bar low and anchors there. Don't average the two questions; a skill must pass both.
...
The loop above is right-sized for personal and team skills. Escalate when the stakes do:

- **Skills shared beyond the builder:** grow to 5–8 on-target prompts across the audience's real range and 3–4 near-misses, and run the health check (`references/health-check.md`) before the skill ships. The wider the audience, the higher the bar.
...
- **The retire check:** occasionally run an on-target prompt *without* the skill. If the bare model now matches the skill's output, the skill has been absorbed and is pure maintenance cost — recommend retiring it. This applies to capability skills; preference skills (your team's way of doing things) don't expire this way.
