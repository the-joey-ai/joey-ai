...
3. The audience runs the skill on a platform with subagents (Claude Code, Cursor, Codex). If the audience includes platforms without them, the skill needs an explicit sequential fallback, which doubles the maintenance surface. Say so before choosing this rung.
...
When two skills genuinely share a workflow, they share **process artifacts**: a record one creates and the other reads, a thread one opens and the other posts to, files in a folder both know about. The artifact is durable, inspectable, and survives either skill breaking. (A submission skill that creates an intake record and a separate review skill that reads it is the canonical shape of this pattern.)
...
1. **Path scan.** Search every file in the bundle for absolute paths (`/Users/`, `/mnt/`, `C:\`, `~/`) and for references to files by name. Every hit either points inside the bundle (relative path) or gets the content pulled *into* the bundle.
