---
name: wrap-session
description: "Closes a working session on a project folder that carries this method's documents by banking everything before it evaporates: SPEC.md trued up to the honest state including half-done steps, anything cut or deferred moved into ENHANCEMENTS.md, REQUIREMENTS known limitations amended if reality bit, SECURITY.md checked and the diff swept for secrets, a session-log entry written in AGENTS.md, then commit, and push if there is a remote. Use when a session is ending — \"let's wrap\", \"done for today\", \"that's all for now\", \"I'm heading out\", \"bank this\" — or when the command is invoked, and offer it the moment the user signals they're finishing. Not for opening a session (start-session) and not for a folder without the documents (establish)."
---
...
**Do the work rather than interviewing about it.** Everything that follows you can determine yourself from the session, the diff, and the documents. Write it, then report what you wrote in a few lines. Someone who says "let's wrap" is telling you they're finished talking.
...
That sentence is worth more than every checked box in the file, because it's what stops the next session either redoing finished work or trusting something that doesn't work. The test: could start-session orient a stranger from this file alone?
...
- **Next up** — the two or three things that genuinely come next. start-session reads this first when a phase finishes, so it is the one section that must stay current and short.
...
> This is where the version you actually wanted lives while the agent builds the version that ships.
...
Prepend to the session log at the bottom of the project's `AGENTS.md`, newest first:
...
## 5. Commit, and push if there is a remote

One commit unless the session had genuinely separable pieces. Message says what the session did in plain words. If the project has a remote, **push, and verify it landed** (`git status` clean and up to date with origin): the push is part of the wrap, not a proposal, because work that only exists on one machine is one disk failure from gone. Never force-push; if the push fails (auth, network, diverged), fixing that *is* the remaining work of the session. If there is no remote, the commit is the wrap; say so in one line.

## Close out

One or two sentences: where the project stands now, and what the next session probably starts with. That's the handoff — to the owner's future self, most likely on a different device.
...
- **The session log lives in `AGENTS.md`, not `CLAUDE.md`.** If this project still keeps its log in `CLAUDE.md`, it predates the method; write the entry where the log already is and mention adoption once.
