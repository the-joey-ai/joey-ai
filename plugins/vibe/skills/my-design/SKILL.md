---
name: my-design
description: "Create or evolve the user's master DESIGN.md — their personal design signature, kept at ~/Claude/my-design/DESIGN.md and derived into every project they build. Use when they want to define or record their design taste, start their master design file, or process its harvest inbox after a project — “set up my DESIGN.md”, “what's my design identity”, “add this to my design file”, “I want my things to look like they came from the same person”. Also use when an old project design doc was really their taste and should become the master. Not for designing one specific project's look, which /vibe:new-project derives from the master, and not for reviewing or critiquing an interface that already exists."
---
...
Taste shows up in everything the user makes, it sharpens with each build, and it is what makes a set of projects feel like a considered set rather than five unrelated things. So the master lives in one place — `~/Claude/my-design/DESIGN.md` — and each project *derives* from it rather than inventing its own look.
...
- **No file** → first run. **Read `references/first-run-interview.md`** and run the interview it describes, then write the file per the rules below. Creating the folder for the first time: `git init -b main` it and create the private repo (`gh repo create <owner>/my-design --private --source . --push` after the first commit) — the master follows the same rule as everything else: it lives on GitHub, so cloud sessions and other machines can reach it.
- **File exists** → evolve. **Read `references/evolve-mode.md`** and work through it: the harvest inbox, whatever the user came with, and (occasionally) a tightening pass. Push when done.
...
**Write values, not vibes.** "Warm off-white, `#F8F4EF`, never pure white" is a rule that can be followed and checked. "Warm, minimal aesthetic" is not. Where the user doesn't know a value, say what you'd pick and why, and let them accept or change it — a concrete default they can react to beats an open question they can't answer.
...
- **Existing raw material is better than a blank page.** A past project's design doc, a card template, a personality contract in an app's docs — these are real records of the user's taste. Ask what exists and offer to mine it on a first run rather than starting cold.
