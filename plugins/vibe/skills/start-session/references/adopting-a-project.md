...
The playbook for bringing a project that predates vibe — or was built without it — into the document system. Most working folders are like this: projects with rich, converged CLAUDE.md files of their own, and code projects with doc sets under different names.
...
## Do it with the user, not at them
...
This is the system's hardest rule, because the existing documents are good. An existing CLAUDE.md with a File Inventory, Active State, and Session Log is already doing most of CLAUDE-project.md's job — adoption **reshapes and fills gaps**, it does not replace a working document with a fresh template. Concretely:
...
- An unbounded session log gets the rolling-window treatment (three entries here, the rest to `docs/session-history.md`) — a CLAUDE.md that has grown to hundreds of kilobytes is the cautionary tale, and it is fixed exactly this way.
...
**`REQUIREMENTS.md`.** The north star comes from the user's mouth, not the feature list — ask what the project is *for* and write that. The v1 list is whatever shipped, checked off (`- [x]`), which reads as history rather than plan and is correct. Constraints the project has always lived with go in Known limitations even though they were never "hit" during a session.
...
**`SECURITY.md`.** Often the most valuable document of the pass, because a running project has usually *made* its security decisions without writing them down. Write what data it actually holds (read the tree, not the README's claims), where every credential lives by name, and — for anything that might ever go public — what the history contains. Finding credentials hardcoded, moving them to `secrets.env`, and listing what to rotate is what this looks like done fully.
